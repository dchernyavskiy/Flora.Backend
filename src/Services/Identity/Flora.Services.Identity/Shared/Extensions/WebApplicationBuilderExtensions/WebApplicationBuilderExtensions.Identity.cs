...
namespace Flora.Services.Identity.Shared.Extensions.WebApplicationBuilderExtensions;
    ...
    )
    {
        if (builder.Configuration.GetValue<bool>("PostgresOptions:UseInMemory"))
        {
            builder.Services.AddDbContext<IdentityContext>(
                options => options.UseInMemoryDatabase("Shop.Services.Flora.Services.Identity")
            );
        ...
        // Problem with .net core identity - will override our default authentication scheme `JwtBearerDefaults.AuthenticationScheme` to unwanted `Flora.Services.Identity.Application` in `AddIdentity()` method .net identity
