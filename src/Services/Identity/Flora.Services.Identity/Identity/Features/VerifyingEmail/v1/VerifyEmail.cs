...
using Flora.Services.Identity.Identity.Features.VerifyingEmail.v1.Exceptions;
...
namespace Flora.Services.Identity.Identity.Features.VerifyingEmail.v1;
...
internal class VerifyEmailHandler : ICommandHandler<VerifyEmail>
{
    ...
    public async Task<Unit> Handle(VerifyEmail request, CancellationToken cancellationToken)
    {
        ...
        if (user == null)
        {
            throw new IdentityUserNotFoundException(request.Email);
        }
