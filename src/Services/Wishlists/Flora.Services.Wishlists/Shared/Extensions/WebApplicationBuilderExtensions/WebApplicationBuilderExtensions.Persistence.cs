...
using Flora.Services.Wishlists.Shared.Data;
using Microsoft.EntityFrameworkCore;

namespace Flora.Services.Wishlists.Shared.Extensions.WebApplicationBuilderExtensions;
...
public static partial class WebApplicationBuilderExtensions
{
    ...
    private static void AddPostgresWriteStorage(IServiceCollection services, IConfiguration configuration)
    {
        if (configuration.GetValue<bool>("PostgresOptions:UseInMemory"))
        {
            ...
            services.AddScoped<IDbFacadeResolver>(provider => provider.GetService<WishlistDbContext>()!);
        }
        else
        {
            services.AddPostgresDbContext<WishlistDbContext>();
        }

        services.AddScoped<IWishlistDbContext>(provider => provider.GetRequiredService<WishlistDbContext>());
    }
