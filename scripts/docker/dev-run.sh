...
if [ -z "$(docker ps -q -f name=${container_name})" ]; then
    docker run -it --rm --publish $http_port:80 --publish $https_port:443 --publish-all --env-file $env_path --network=flora --name $container_name --mount type=bind,src=${HOME}/vsdbg,dst=/vsdbg --mount type=bind,source=${HOME}/.nuget/packages,destination=/root/.nuget/packages,readonly --mount type=bind,source=${HOME}/.nuget/packages,destination=/home/appuser/.nuget/packages,readonly --mount type=bind,src=${HOME}/.aspnet/https,dst=/https,readonly $image_name
fi
